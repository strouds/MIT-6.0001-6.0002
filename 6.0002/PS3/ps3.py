...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        # TODO: does this need to covert from Position class to a tuple?
    ...
    def is_position_in_room(self, pos):
        ...
        """
        return (math.floor(pos.get_x()), math.floor(pos.get_y())) in self.tiles.keys()
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.direction = random.random() * 360 # random float from 0-360
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        return (math.floor(pos.get_x()), math.floor(pos.get_y())) in self.tiles and \
            not self.is_position_furnished(pos)
...
class BouncyRobot(Robot):
    """ moves in a straight line until hitting a wall, then rotates 90 degrees """
    def update_position_and_clean(self):
        new_pos = self.pos.get_new_position(self.direction, self.speed)
        if self.room.is_position_valid(new_pos):
            self.set_robot_position(new_pos)
            ...
            test_pos_turn = self.pos.get_new_position(self.get_robot_direction() + 90,
                                                     self.speed)
            if self.room.is_position_valid(test_pos_turn):
                self.set_robot_direction((self.get_robot_direction() + 90) 
                                         * (random.random() * .5 + 1))
            else:
                self.set_robot_direction((self.get_robot_direction() - 90) 
                                         * (random.random() * .5 + 1))
# test_robot_movement(BouncyRobot, EmptyRoom)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    """
    def trial():
        """ room is EmptyRoom, robots is a list of robots """
...
# print ('avg time steps: ' + str(run_simulation(3, 1.0, 1, 20, 20, 3, 0.8, 1, StandardRobot)))
...
#       of a 20x20 room?
#
#   FaultyRobot seems to consistently take about 20% more time to clean a room.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#
#   Both types of robots' performance suffered as the aspect ratio deviated from
#       1, but the FaultyRobot suffered more.
...
def show_plot_room_shape(title, x_label, y_label):
    ...
    for width in [10, 20, 25, 50]:
        ...
        times1.append(run_simulation(2, 1.0, 1, width, height, 3, 0.8, 100, StandardRobot))
        times2.append(run_simulation(2, 1.0, 1, width, height, 3, 0.8, 100, BouncyRobot))
